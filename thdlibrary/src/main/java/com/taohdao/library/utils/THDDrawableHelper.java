 ...
 * @date 2016-03-17
 */
public class THDDrawableHelper {
    private static final String TAG = THDDrawableHelper.class.getSimpleName();
    ...
    public static Bitmap createBitmapFromView(View view, int leftCrop, int topCrop, int rightCrop, int bottomCrop) {
        Bitmap originBitmap = THDDrawableHelper.createBitmapFromView(view);
    ...
    public static BitmapDrawable createDrawableWithSize(Resources resources, int width, int height, int cornerRadius, @ColorInt int filledColor) {
        ...
        if (cornerRadius > 0) {
            ...
            textPaint.setColor(resources.getColor(R.color.thd_config_color_white));

            // 计算Baseline绘制的起点X轴坐标 ，计算方式：画布宽度的一半 - 文字宽度的一半
            int baseX = (int) (canvas.getWidth() / 2 - textPaint.measureText("促销") / 2);
            // 计算Baseline绘制的Y坐标 ，计算方式：画布高度的一半 - 文字总高度的一半
            int baseY = (int) ((canvas.getHeight() / 2) - ((textPaint.descent() + textPaint.ascent()) / 2));

            canvas.drawRoundRect(new RectF(0, 0, width, height), cornerRadius, cornerRadius, paint);
            canvas.drawText("促销",baseX,baseY,textPaint);
        } else {
     ...
     * @return 指定大小的纯色图片
     */
    public static BitmapDrawable createTextDrawableWithSize(Resources resources, int width, int height, int cornerRadius, @ColorInt int filledColor,String text,float textSize) {
        Bitmap output = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(output);
