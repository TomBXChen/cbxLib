...
import com.taohdao.library.widget.bottomnavigation.behaviour.BottomNavBarFabBehaviour;
import com.taohdao.library.widget.bottomnavigation.behaviour.BottomVerticalScrollBehavior;
...
/**
 * Class description : This class is used to draw the layouts and this acts like a bridge between
...
public class BottomNavigationBar extends FrameLayout {
    ...
    public static final int MODE_DEFAULT_FIXED = 5;

    @IntDef({MODE_DEFAULT, MODE_FIXED, MODE_SHIFTING, MODE_FIXED_NO_TITLE, MODE_SHIFTING_NO_TITLE, MODE_DEFAULT_FIXED})
     ...
     * @param attrs   attributes mentioned in the layouts XML by user
     */
    ...
    /**
     * This method initiates the bottomNavigationBar and handles layouts related values
     */
    private void init() {
...
//    }
    ...
    public void initialiseDelay() {
        getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {

            @Override
            public void onGlobalLayout() {
                getViewTreeObserver().removeOnGlobalLayoutListener(this);
                initialise(getMeasuredWidth());
            }
        });

    }

    public void initialise(int screenWidth) {
        ...
        if (!mBottomNavigationItems.isEmpty()) {
            ...
            } else if (mMode == MODE_DEFAULT_FIXED) {
                int[] widths = BottomNavigationHelper.getMeasurementsForFixedMode(getContext(), screenWidth, mBottomNavigationItems.size(), mScrollable);
                int itemWidth = widths[0];

                for (BottomNavigationItem currentItem : mBottomNavigationItems) {
                    DefaultNavigationTab bottomNavigationTab = new DefaultNavigationTab(getContext());
                    setUpTab(mMode == MODE_FIXED_NO_TITLE, bottomNavigationTab, currentItem, itemWidth, itemWidth);
                }
            }
...
//                            }
        ...
        if (callListener && mTabChangeListener != null) {
            mTabChangeListener.onTabSelected(newPosition);
        }
    }
...
//                && oldPosition != -1) {
    ...
    public abstract static class OnTabChangeListener implements OnTabSelectedListener {
