package com.taohdao.library.common.widget.floatMenu;
...
public class FloatMenu extends PopupWindow implements PopupWindow.OnDismissListener {
	...
	public FloatMenu(Context context, View view) {
		...
		this.view = view;
		VERTICAL_OFFSET = SizeUtils.dp2px( 10);
		DEFAULT_MENU_WIDTH = SizeUtils.dp2px( 180);
		final DisplayMetrics displayMetrics = Utils.getApp().getResources().getDisplayMetrics();
		screenPoint = new Point(displayMetrics.widthPixels,displayMetrics.heightPixels);
	...
	private void generateLayout(int itemWidth) {
		...
		for(int i = 0; i < menuItemList.size(); i ++){
			...
			if(menuModel.getItemResId() != View.NO_ID){
				...
				textView.setCompoundDrawablePadding(SizeUtils.dp2px(12));
...
//		menuItemList.add(String.valueOf(itemTitle));
	...
	public void show(Point point,OnPopDismissListener onDismissListener){
		clickX = point.x;
		clickY = point.y;
		setOnPopDismissListener(onDismissListener);
		show();
	...
	public void setOnPopDismissListener(OnPopDismissListener mOnPopDismissListener) {
		this.mOnPopDismissListener = mOnPopDismissListener;
	}
