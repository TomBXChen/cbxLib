...
import com.taohdao.library.common.checkVersionlib.core.http.FileCallBack;
...
public class DownloadManager {
    ...
    public static void downloadAPK(final String url, final VersionParams versionParams, final DownloadListener listener) {
        ...
        String downloadPath = versionParams.getDownloadAPKPath() +         Utils.getApp().getString(R.string.versionchecklib_download_apkname,Utils.getApp().getPackageName() );
        //静默下载也判断本地是否有缓存
        if (versionParams.isSilentDownload()) {

            if (!versionParams.isForceRedownload()) {
                //判断本地文件是否存在
                if (checkAPKIsExists(Utils.getApp(), downloadPath)) {
                    if (listener != null)
                        listener.onCheckerDownloadSuccess(new File(downloadPath));
                    return;
                }
                silentDownloadAPK(Utils.getApp(), url, versionParams, listener);
        ...
        if (!versionParams.isForceRedownload()) {
            ...
            if (checkAPKIsExists(Utils.getApp(), downloadPath)) {
                if (listener != null)
                    listener.onCheckerDownloadSuccess(new File(downloadPath));
                AppUtils.installApk(Utils.getApp(), new File(downloadPath));
                return;
        ...
        if (versionParams.isShowNotification()) {
            ...
            builder = createNotification(Utils.getApp());
        ...
        AllenHttp.getHttpClient().newCall(request).enqueue(new FileCallBack(versionParams.getDownloadAPKPath(), Utils.getApp().getString(R.string.versionchecklib_download_apkname,Utils.getApp().getPackageName() )) {
            @Override
            public void onSuccess(File file, Call call, Response response) {
                ...
                if (versionParams.isShowNotification()) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                        uri = VersionFileProvider.getUriForFile(Utils.getApp(), Utils.getApp().getPackageName() + ".versionProvider", file);
//                        uri = VersionFileProvider.getUriForFile(Utils.getApp(), "com.taohdao.library.common.checkVersionlib.core.VersionFileProvider", file);
                        ALog.e(Utils.getApp().getPackageName() + "");
                    ...
                    PendingIntent pendingIntent = PendingIntent.getActivity(Utils.getApp(), 0, i, 0);
                    finalBuilder.setContentIntent(pendingIntent);
                    finalBuilder.setContentText(Utils.getApp().getString(R.string.versionchecklib_download_finish));
                    finalBuilder.setProgress(100, 100, false);
                    finalManager.cancelAll();
                    finalManager.notify(0, finalBuilder.build());
                }

                AppUtils.installApk(Utils.getApp(), file);
            }
...
//                showLoadingDialog(currentProgress);
                ...
                if (currentProgress - lastProgress >= 5) {
                    ...
                    if (versionParams.isShowNotification() && !isDownloadSuccess) {
                        ...
                        finalBuilder.setContentText(String.format(Utils.getApp().getString(R.string.versionchecklib_download_progress), lastProgress));
...
//                        finalBuilder.setDefaults(0);
            ...
            public void onDownloadFailed() {
                if (versionParams.isShowNotification()) {
                    Intent intent = new Intent(Utils.getApp(), versionParams.getCustomDownloadActivityClass());
                    intent.putExtra("isRetry", true);
//                intent.putExtra(AVersionService.VERSION_PARAMS_KEY, paramBundle);
                    intent.putExtra(AVersionService.VERSION_PARAMS_KEY, versionParams);
                    intent.putExtra("downloadUrl", url);
                    PendingIntent pendingIntent = PendingIntent.getActivity(Utils.getApp(), 0, intent, FLAG_UPDATE_CURRENT);
                    finalBuilder.setContentIntent(pendingIntent);
                    finalBuilder.setContentText(Utils.getApp().getString(R.string.versionchecklib_download_fail));
