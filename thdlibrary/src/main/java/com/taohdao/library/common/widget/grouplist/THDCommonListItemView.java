...
/**
 * 作为通用列表 {@link THDGroupListView} 里的 item 使用，也可以单独使用。
 * 支持以下样式:
 *
 * @author chantchen
 * @date 2015-1-8
 */
public class THDCommonListItemView extends RelativeLayout {
    ...
    public THDCommonListItemView(Context context) {
        this(context, null);
    }

    public THDCommonListItemView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.THDCommonListItemViewStyle);
    }

    public THDCommonListItemView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs, defStyleAttr);
    }

    protected void init(Context context, AttributeSet attrs, int defStyleAttr) {
        LayoutInflater.from(context).inflate(R.layout.thd_common_list_item, this, true);

        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.THDCommonListItemView, defStyleAttr, 0);
        @QMUICommonListItemOrientation int orientation = array.getInt(R.styleable.THDCommonListItemView_thd_orientation, HORIZONTAL);
        @QMUICommonListItemAccessoryType int accessoryType = array.getInt(R.styleable.THDCommonListItemView_thd_accessory_type, ACCESSORY_TYPE_NONE);
        final int initTitleColor = array.getColor(R.styleable.THDCommonListItemView_thd_commonList_titleColor, THDResHelper.getAttrColor(getContext(), R.attr.thd_config_color_gray_1));
        final int initDetailColor = array.getColor(R.styleable.THDCommonListItemView_thd_commonList_detailColor, THDResHelper.getAttrColor(getContext(), R.attr.thd_config_color_gray_5));
        array.recycle();
        ...
        if (THDViewHelper.getIsLastLineSpacingExtraError()) {
            detailTextViewLP.bottomMargin = -THDResHelper.getAttrDimen(context, R.attr.thd_common_list_item_detail_line_space);
        }
        if (orientation == VERTICAL) {

            detailTextViewLP.topMargin =    SizeUtils.dp2px(6);
        } else {
    ...
    public void setOrientation(@QMUICommonListItemOrientation int orientation) {
        ...
        if (mOrientation == VERTICAL) {
            ...
            spaceLp.height = SizeUtils.dp2px(4);
            spaceLp.weight = 0;
            mTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, THDResHelper.getAttrDimen(getContext(), R.attr.thd_common_list_item_title_v_text_size));
            mDetailTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, THDResHelper.getAttrDimen(getContext(), R.attr.thd_common_list_item_detail_v_text_size));
        } else {
            ...
            spaceLp.height = 0;
            spaceLp.weight = 1;
            mTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, THDResHelper.getAttrDimen(getContext(), R.attr.thd_common_list_item_title_h_text_size));
            mDetailTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, THDResHelper.getAttrDimen(getContext(), R.attr.thd_common_list_item_detail_h_text_size));
        }
    }
    ...
    public void setAccessoryType(@QMUICommonListItemAccessoryType int type) {
        ...
        switch (type) {
            ...
            case ACCESSORY_TYPE_CHEVRON: {
                ...
                tempImageView.setImageDrawable(THDResHelper.getAttrDrawable(getContext(), R.attr.thd_common_list_item_chevron));
            ...
            case ACCESSORY_TYPE_SWITCH: {
                if (mSwitch == null) {
                    ...
                    mSwitch.setButtonDrawable(THDResHelper.getAttrDrawable(getContext(), R.attr.thd_common_list_item_switch));
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (mRedDot != null && mRedDot.getVisibility() == View.VISIBLE) {
            ...
            if (mRedDotPosition == REDDOT_POSITION_LEFT) {
                ...
                left = (int) (textLeft + textWidth + SizeUtils.dp2px( 4)); // 在原来红点位置的基础上右移
        ...
        if (mNewTip != null && mNewTip.getVisibility() == View.VISIBLE) {
            int textLeft = mTextContainer.getLeft();
            float textWidth = mTextView.getPaint().measureText(mTextView.getText().toString()); // 文字宽度
            int left = (int) (textLeft + textWidth + SizeUtils.dp2px( 4)); // 在原来红点位置的基础上右移
