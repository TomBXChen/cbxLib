...
import com.taohdao.library.utils.THDCollapsingTextHelper;
import com.taohdao.library.utils.THDInterpolatorStaticHolder;
import com.taohdao.library.utils.THDLangHelper;
import com.taohdao.library.utils.THDViewOffsetHelper;
...
/**
 * 参考 {@link android.support.design.widget.CollapsingToolbarLayout}, 适配 THDTopBar
 *
 * @author cginechen
 * @date 2017-09-02
 */

public class THDCollapsingTopBarLayout extends FrameLayout implements IWindowInsetLayout {

    private static final int DEFAULT_SCRIM_ANIMATION_DURATION = 600;

    private boolean mRefreshToolbar = true;
    private int mTopBarId;
    private THDTopBar mTopBar;
    ...
    final THDCollapsingTextHelper mCollapsingTextHelper;
    ...
    public THDCollapsingTopBarLayout(Context context) {
        this(context, null);
    }

    public THDCollapsingTopBarLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public THDCollapsingTopBarLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        mCollapsingTextHelper = new THDCollapsingTextHelper(this);
        mCollapsingTextHelper.setTextSizeInterpolator(THDInterpolatorStaticHolder.DECELERATE_INTERPOLATOR);

        THDViewHelper.checkAppCompatTheme(context);

        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.THDCollapsingTopBarLayout, defStyleAttr, 0);

        mCollapsingTextHelper.setExpandedTextGravity(
                a.getInt(R.styleable.THDCollapsingTopBarLayout_thd_expandedTitleGravity,
                        Gravity.CENTER_HORIZONTAL | Gravity.BOTTOM));
        mCollapsingTextHelper.setCollapsedTextGravity(
                a.getInt(R.styleable.THDCollapsingTopBarLayout_thd_collapsedTitleGravity,
                        GravityCompat.START | Gravity.CENTER_VERTICAL));


        mExpandedMarginStart = mExpandedMarginTop = mExpandedMarginEnd = mExpandedMarginBottom =
                a.getDimensionPixelSize(R.styleable.THDCollapsingTopBarLayout_thd_expandedTitleMargin, 0);

        if (a.hasValue(R.styleable.THDCollapsingTopBarLayout_thd_expandedTitleMarginStart)) {
            mExpandedMarginStart = a.getDimensionPixelSize(
                    R.styleable.THDCollapsingTopBarLayout_thd_expandedTitleMarginStart, 0);
        }
        if (a.hasValue(R.styleable.THDCollapsingTopBarLayout_thd_expandedTitleMarginEnd)) {
            mExpandedMarginEnd = a.getDimensionPixelSize(
                    R.styleable.THDCollapsingTopBarLayout_thd_expandedTitleMarginEnd, 0);
        }
        if (a.hasValue(R.styleable.THDCollapsingTopBarLayout_thd_expandedTitleMarginTop)) {
            mExpandedMarginTop = a.getDimensionPixelSize(
                    R.styleable.THDCollapsingTopBarLayout_thd_expandedTitleMarginTop, 0);
        }
        if (a.hasValue(R.styleable.THDCollapsingTopBarLayout_thd_expandedTitleMarginBottom)) {
            mExpandedMarginBottom = a.getDimensionPixelSize(
                    R.styleable.THDCollapsingTopBarLayout_thd_expandedTitleMarginBottom, 0);
        }

        mCollapsingTitleEnabled = a.getBoolean(R.styleable.THDCollapsingTopBarLayout_thd_titleEnabled, true);
        setTitle(a.getText(R.styleable.THDCollapsingTopBarLayout_thd_title));

        // First load the default text appearances
        mCollapsingTextHelper.setExpandedTextAppearance(R.style.THD_CollapsingTopBarLayoutExpanded);
        mCollapsingTextHelper.setCollapsedTextAppearance(R.style.THD_CollapsingTopBarLayoutCollapsed);

        // Now overlay any custom text appearances
        if (a.hasValue(R.styleable.THDCollapsingTopBarLayout_thd_expandedTitleTextAppearance)) {
            mCollapsingTextHelper.setExpandedTextAppearance(
                    a.getResourceId(R.styleable.THDCollapsingTopBarLayout_thd_expandedTitleTextAppearance, 0));
        }
        if (a.hasValue(R.styleable.THDCollapsingTopBarLayout_thd_collapsedTitleTextAppearance)) {
            mCollapsingTextHelper.setCollapsedTextAppearance(
                    a.getResourceId(R.styleable.THDCollapsingTopBarLayout_thd_collapsedTitleTextAppearance, 0));
        }

        mScrimVisibleHeightTrigger = a.getDimensionPixelSize(
                R.styleable.THDCollapsingTopBarLayout_thd_scrimVisibleHeightTrigger, -1);

        mScrimAnimationDuration = a.getInt(
                R.styleable.THDCollapsingTopBarLayout_thd_scrimAnimationDuration,
                DEFAULT_SCRIM_ANIMATION_DURATION);

        setContentScrim(a.getDrawable(R.styleable.THDCollapsingTopBarLayout_thd_contentScrim));
        setStatusBarScrim(a.getDrawable(R.styleable.THDCollapsingTopBarLayout_thd_statusBarScrim));

        mTopBarId = a.getResourceId(R.styleable.THDCollapsingTopBarLayout_thd_topBarId, -1);
    ...
    private void ensureToolbar() {
        ...
        if (mTopBarId != -1) {
            ...
            mTopBar = (THDTopBar) findViewById(mTopBarId);
            if (mTopBar != null) {
        ...
        if (mTopBar == null) {
            ...
            THDTopBar topBar = null;
            for (int i = 0, count = getChildCount(); i < count; i++) {
                final View child = getChildAt(i);
                if (child instanceof THDTopBar) {
                    topBar = (THDTopBar) child;
                    break;
                }
            }
            mTopBar = topBar;
    ...
    /**
     * Returns the direct child of this layouts, which itself is the ancestor of the
     * given view.
     */
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mCollapsingTitleEnabled) {
            ...
            THDViewHelper.getDescendantRect(this, mTopBar, mTmpRect);
//            mTmpRect.top = mTmpRect.top - topBarInsetAdjustTop;
    ...
    static THDViewOffsetHelper getViewOffsetHelper(View view) {
        THDViewOffsetHelper offsetHelper = (THDViewOffsetHelper) view.getTag(R.id.thd_view_offset_helper);
        if (offsetHelper == null) {
            offsetHelper = new THDViewOffsetHelper(view);
            view.setTag(R.id.thd_view_offset_helper, offsetHelper);
        }
        return offsetHelper;
    }
        ...
        if (mScrimAnimator == null) {
            ...
            mScrimAnimator.setInterpolator(
                    targetAlpha > mScrimAlpha
                            ? THDInterpolatorStaticHolder.FAST_OUT_LINEAR_IN_INTERPOLATOR
                            : THDInterpolatorStaticHolder.LINEAR_OUT_SLOW_IN_INTERPOLATOR);
    ...
    void setScrimAlpha(int alpha) {
        if (alpha != mScrimAlpha) {
            ...
            mScrimAlpha = alpha;
            ViewCompat.postInvalidateOnAnimation(THDCollapsingTopBarLayout.this);
        }
    }
    ...
    protected boolean checkLayoutParams(ViewGroup.LayoutParams p) {
        return p instanceof THDCollapsingTopBarLayout.LayoutParams;
    }

    @Override
    protected THDCollapsingTopBarLayout.LayoutParams generateDefaultLayoutParams() {
        return new THDCollapsingTopBarLayout.LayoutParams(THDCollapsingTopBarLayout.LayoutParams.MATCH_PARENT, THDCollapsingTopBarLayout.LayoutParams.MATCH_PARENT);
    }

    @Override
    public FrameLayout.LayoutParams generateLayoutParams(AttributeSet attrs) {
        return new THDCollapsingTopBarLayout.LayoutParams(getContext(), attrs);
    }

    @Override
    protected FrameLayout.LayoutParams generateLayoutParams(ViewGroup.LayoutParams p) {
        return new THDCollapsingTopBarLayout.LayoutParams(p);
    }
    ...
    public boolean applySystemWindowInsets19(Rect insets) {
        ...
        if (!THDLangHelper.objectEquals(mLastInsets, newInsets)) {
    ...
    public static class LayoutParams extends FrameLayout.LayoutParams {
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            ...
            TypedArray a = c.obtainStyledAttributes(attrs,
                    R.styleable.THDCollapsingTopBarLayout_Layout);
            mCollapseMode = a.getInt(
                    R.styleable.THDCollapsingTopBarLayout_Layout_thd_layout_collapseMode,
                    COLLAPSE_MODE_OFF);
            setParallaxMultiplier(a.getFloat(
                    R.styleable.THDCollapsingTopBarLayout_Layout_thd_layout_collapseParallaxMultiplier,
    ...
    final int getMaxOffsetForPinChild(View child) {
        final THDViewOffsetHelper offsetHelper = getViewOffsetHelper(child);
        final THDCollapsingTopBarLayout.LayoutParams lp = (THDCollapsingTopBarLayout.LayoutParams) child.getLayoutParams();
        return getHeight()
        ...
        public void onOffsetChanged(AppBarLayout layout, int verticalOffset) {
            ...
            for (int i = 0, z = getChildCount(); i < z; i++) {
                ...
                switch (lp.mCollapseMode) {
                    case THDCollapsingTopBarLayout.LayoutParams.COLLAPSE_MODE_PIN:
                        offsetHelper.setTopAndBottomOffset(
                                THDLangHelper.constrain(-verticalOffset, 0, getMaxOffsetForPinChild(child)));
                        break;
                    case THDCollapsingTopBarLayout.LayoutParams.COLLAPSE_MODE_PARALLAX:
                        offsetHelper.setTopAndBottomOffset(
            ...
            final int expandRange = getHeight() - ViewCompat.getMinimumHeight(
                    THDCollapsingTopBarLayout.this) - insetTop;
