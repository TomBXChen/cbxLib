...
public class THDLoadingView  extends View {
    ...
    public THDLoadingView(Context context) {
        this(context, null);
    }

    public THDLoadingView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.THDLoadingStyle);
    }

    public THDLoadingView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.THDLoadingView, defStyleAttr, 0);
        mSize = array.getDimensionPixelSize(R.styleable.THDLoadingView_thd_loading_view_size, SizeUtils.dp2px(32));
        mPaintColor = array.getInt(R.styleable.THDLoadingView_android_color, Color.WHITE);
        array.recycle();
        initPaint();
    }

    public THDLoadingView(Context context, int size, int color) {
        super(context);
        mSize = size;
        mPaintColor = color;
        initPaint();
