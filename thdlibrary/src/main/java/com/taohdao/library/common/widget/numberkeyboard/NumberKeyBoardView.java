...
public class NumberKeyBoardView extends KeyboardView {
    ...
    public static final int TYPE_DECIMALS = 2;
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (keys != null) {
            for (Key key : keys) {
                ...
                switch (type){
                    case TYPE_NUMBER:
                        if(key.codes[0] == 99||key.codes[0] ==46){
                            drawKeyBackground(R.drawable.bg_keyboardview_none, canvas, key);
                        }
                        break;
                    case TYPE_DECIMALS:
                        if(key.codes[0] == 99){
    ...
    private void drawKeyBackground(int drawableId, Canvas canvas, Key key) {
        ...
        npd.setBounds(key.x, key.y+2, key.x + key.width-2, key.y
                + key.height);
        npd.draw(canvas);
    }
