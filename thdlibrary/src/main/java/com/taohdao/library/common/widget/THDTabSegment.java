...
import com.taohdao.library.utils.THDColorHelper;
import com.taohdao.library.utils.THDDrawableHelper;
 ...
 * @date 2016-01-27
 */
public class THDTabSegment extends HorizontalScrollView {
    // icon position
...
//    private AdapterChangeListener mAdapterChangeListener;
    ...
    public THDTabSegment(Context context) {
        this(context, null);
    }

    public THDTabSegment(Context context, boolean hasIndicator) {
        this(context, null);
        mHasIndicator = hasIndicator;
    }

    public THDTabSegment(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.THDTabSegmentStyle);
    }

    public THDTabSegment(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mDefaultSelectedColor = THDResHelper.getAttrColor(context, R.attr.qmui_config_color_blue);
        mDefaultNormalColor = ContextCompat.getColor(context, R.color.thd_config_color_gray_5);
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.THDTabSegment, defStyleAttr, 0);
        mHasIndicator = array.getBoolean(R.styleable.THDTabSegment_thd_tab_has_indicator, true);
        mIndicatorHeight = array.getDimensionPixelSize(R.styleable.THDTabSegment_thd_tab_indicator_height,
                getResources().getDimensionPixelSize(R.dimen.thd_tab_segment_indicator_height));
        mTabTextSize = array.getDimensionPixelSize(R.styleable.THDTabSegment_android_textSize,
                getResources().getDimensionPixelSize(R.dimen.thd_tab_segment_text_size));
        mIndicatorTop = array.getBoolean(R.styleable.THDTabSegment_thd_tab_indicator_top, false);
        mTabIconPosition = array.getInt(R.styleable.THDTabSegment_thd_tab_icon_position, ICON_POSITION_LEFT);
        mMode = array.getInt(R.styleable.THDTabSegment_thd_tab_mode, MODE_FIXED);
        mItemSpaceInScrollMode = array.getDimensionPixelSize(R.styleable.THDTabSegment_thd_tab_space, SizeUtils.dp2px(10));
        typefaceProviderName = array.getString(R.styleable.THDTabSegment_thd_tab_typeface_provider);
        array.recycle();
    ...
    private void createTypefaceProvider(Context context, String className) {
        if (THDLangHelper.isNullOrEmpty(className)) {
            return;
        }
    ...
    public THDTabSegment addTab(Tab item) {
    ...
    private void createIndicatorView() {
        if (mIndicatorView == null) {
            ...
            if (mIndicatorDrawable != null) {
                THDViewHelper.setBackgroundKeepingPadding(mIndicatorView, mIndicatorDrawable);
            } else {
...
//            viewPager.addOnAdapterChangeListener(mAdapterChangeListener);
    ...
    private void changeTabColor(TextView textView, int color, Tab model, int status) {
        ...
        THDDrawableHelper.setDrawableTintColor(drawable, color);
        setDrawable(textView, model.getNormalIcon(), getTabIconPosition(model));
    }
        ...
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                if (mIndicatorView != null && listViews.size() > 1) {
                    ...
                    if (mIndicatorDrawable == null) {
                        mIndicatorView.setBackgroundColor(THDColorHelper.computeColor(getTabSelectedColor(prevModel), getTabSelectedColor(nowModel), animValue));
                    }
                    mIndicatorView.layout(targetLeft, mIndicatorView.getTop(), targetLeft + targetWidth, mIndicatorView.getBottom());
                }
                int preColor = THDColorHelper.computeColor(getTabSelectedColor(prevModel), getTabNormalColor(prevModel), animValue);
                int nowColor = THDColorHelper.computeColor(getTabNormalColor(nowModel), getTabSelectedColor(nowModel), animValue);
    ...
    public void updateIndicatorPosition(int index, float offsetPercent) {
        ...
        int preColor = THDColorHelper.computeColor(getTabSelectedColor(preModel), getTabNormalColor(preModel), offsetPercent);
        int targetColor = THDColorHelper.computeColor(getTabNormalColor(targetModel), getTabSelectedColor(targetModel), offsetPercent);
        ...
        if (mIndicatorView != null && listViews.size() > 1) {
            ...
            if (mIndicatorDrawable == null) {
                int indicatorColor = THDColorHelper.computeColor(getTabSelectedColor(preModel), getTabSelectedColor(targetModel), offsetPercent);
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<THDTabSegment> mTabSegmentRef;

        public TabLayoutOnPageChangeListener(THDTabSegment tabSegment) {
        ...
        public void onPageScrolled(final int position, final float positionOffset,
            ...
            final THDTabSegment tabSegment = mTabSegmentRef.get();
            if (tabSegment != null) {
    ...
    public static class Tab {
        ...
        private TextView ensureSignCountView(Context context) {
            if (mSignCountTextView == null) {
                mSignCountTextView = new TextView(context, null, R.attr.thd_tab_sign_count_view);
                RelativeLayout.LayoutParams signCountLp = new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, THDResHelper.getAttrDimen(context, R.attr.thd_tab_sign_count_view_minSize));
                signCountLp.addRule(RelativeLayout.ALIGN_TOP, R.id.thd_tab_segment_item_id);
                signCountLp.addRule(RelativeLayout.RIGHT_OF, R.id.thd_tab_segment_item_id);
        ...
        public void showSignCountView(Context context, int count) {
            ...
            if (count != 0) {
                // 显示未读数
                signCountLp.height = THDResHelper.getAttrDimen(mSignCountTextView.getContext(), R.attr.thd_tab_sign_count_view_minSize_with_text);
                mSignCountTextView.setLayoutParams(signCountLp);
                mSignCountTextView.setMinHeight(THDResHelper.getAttrDimen(mSignCountTextView.getContext(), R.attr.thd_tab_sign_count_view_minSize_with_text));
                mSignCountTextView.setMinWidth(THDResHelper.getAttrDimen(mSignCountTextView.getContext(), R.attr.thd_tab_sign_count_view_minSize_with_text));
                mSignCountTextView.setText(getNumberDigitsFormattingValue(count));
            } else {
                // 显示红点
                signCountLp.height = THDResHelper.getAttrDimen(mSignCountTextView.getContext(), R.attr.thd_tab_sign_count_view_minSize);
                mSignCountTextView.setLayoutParams(signCountLp);
                mSignCountTextView.setMinHeight(THDResHelper.getAttrDimen(mSignCountTextView.getContext(), R.attr.thd_tab_sign_count_view_minSize));
                mSignCountTextView.setMinWidth(THDResHelper.getAttrDimen(mSignCountTextView.getContext(), R.attr.thd_tab_sign_count_view_minSize));
        ...
        public int getSignCount() {
            if (mSignCountTextView != null && !THDLangHelper.isNullOrEmpty(mSignCountTextView.getText())) {
        ...
        private String getNumberDigitsFormattingValue(int number) {
            if (THDLangHelper.getNumberDigits(number) > mSignCountDigits) {
                String result = "";
    ...
    public class TabAdapter extends THDItemViewsAdapter<Tab, TabItemView> {
        ...
        protected void bind(Tab item, TabItemView view, int position) {
            ...
            if (mCustomViews != null && mCustomViews.size() > 0) {
                view.setTag(R.id.thd_view_can_not_cache_tag, true);
            ...
            if (item.getNormalIcon() == null) {
                ...
                if (drawable != null) {
                    ...
                    tv.setCompoundDrawablePadding(SizeUtils.dp2px( 4));
                } else {
                    tv.setCompoundDrawables(null, null, null, null);
                }
            }
    ...
    public class TabItemView extends RelativeLayout {
        ...
        public TabItemView(Context context) {
            ...
            // 用于提供给customView布局用
            mTextView.setId(R.id.thd_tab_segment_item_id);
